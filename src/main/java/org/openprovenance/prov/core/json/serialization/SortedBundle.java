...
public class SortedBundle {
  ...
  public SortedBundle(Bundle bun) {
    ...
    if (namespace.getDefaultNamespace() != null)
      this.namespace.register("default", namespace.getDefaultNamespace());
    this.id = bun.getId();
  ...
  public Bundle toBundle(ProvFactory provFactory) {
    ...
    if (deferredId) {
      // *
      // fixed issue with bundle identifier prefix has to be in bundle
      // in this.namespace are only namespaces from Bundle. But prefix can be in Document level namespace as well!
      // that is why prefixes should be loaded from JSON_CONTEXT_KEY_NAMESPACE
      // *

      String prefix = id.getPrefix();
      Map<String, String> prefixes = namespace.getPrefixes();
      if (prefixes == null || prefixes.get(prefix) == null)
        prefixes = getAttributes().get().get(JSON_CONTEXT_KEY_NAMESPACE).getPrefixes();
