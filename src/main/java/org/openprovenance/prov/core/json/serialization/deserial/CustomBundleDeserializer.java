...
public class CustomBundleDeserializer extends JsonDeserializer<Bundle> {
  ...
  public Bundle deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
    ...
    // fixed issue with bundle bundle id in @id field
    // get bundle id from json key
    sbun.setId(new CustomQualifiedNameDeserializer().deserialize(jsonParser.currentName(), deserializationContext));
    ...
    getAttributes().get().put(JSON_CONTEXT_KEY_NAMESPACE, theBun.getNamespace());
