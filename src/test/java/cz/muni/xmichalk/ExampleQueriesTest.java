...
import static cz.muni.xmichalk.util.NameSpaceConstants.DCT_URI;
...
import cz.muni.xmichalk.queries.GetConnectors;
import cz.muni.xmichalk.queries.GetNodeIds;
import cz.muni.xmichalk.queries.GetNodes;
import cz.muni.xmichalk.queries.GetSubgraphs;
import cz.muni.xmichalk.queries.IQuery;
import cz.muni.xmichalk.queries.TestBundleFits;
...
import cz.muni.xmichalk.querySpecification.findable.DerivationPathFromStartNode;
import cz.muni.xmichalk.querySpecification.findable.FilteredSubgraphs;
import cz.muni.xmichalk.querySpecification.findable.FittingLinearSubgraphs;
import cz.muni.xmichalk.querySpecification.findable.FittingNodes;
import cz.muni.xmichalk.querySpecification.findable.StartNode;
...
import cz.muni.xmichalk.querySpecification.nodeConditions.HasAttr;
import cz.muni.xmichalk.querySpecification.nodeConditions.HasAttrQualifiedNameValue;
import cz.muni.xmichalk.querySpecification.nodeConditions.HasAttrStringValue;
import cz.muni.xmichalk.querySpecification.nodeConditions.HasId;
...
public class ExampleQueriesTest {
    ...
    public void findIdsOfDsConnectorSpecialization() throws AccessDeniedException {
        ...
        IQuery<List<QualifiedNameData>> query = new GetNodeIds(new FittingNodes(
                ...
                new FilteredSubgraphs(
                        new EdgeToNodeCondition(new IsRelation(
                                StatementOrBundle.Kind.PROV_SPECIALIZATION), null,
                                null),
                        new StartNode())
                ...
                new FilteredSubgraphs(
                        new AnyTrue<>(List.of(
                                new EdgeToNodeCondition(
                                        ...
                                        null, false),
                                new EdgeToNodeCondition(new IsRelation(
                                        StatementOrBundle.Kind.PROV_SPECIALIZATION),
                                        null, null))),
                        new FilteredSubgraphs(
                                new EdgeToNodeCondition(
        ...
        IQuery<?> query = new GetNodes(new FittingNodes(
                new IsKind(StatementOrBundle.Kind.PROV_ACTIVITY), new FittingLinearSubgraphs(List.of(
                        new EdgeToNodeCondition(null,
                                new IsKind(StatementOrBundle.Kind.PROV_ACTIVITY), null),
                        new EdgeToNodeCondition(
    ...
    public void findSenderReceiverNodesConnectedToDerivationPath() throws IOException {
        ...
        IQuery<?> query = new GetNodes(new FittingNodes(
                new HasAttrQualifiedNameValue(ATTR_PROV_TYPE.getUri(),
                        CPM_URI + "(sender|receiver)Agent"),
                new FittingLinearSubgraphs(
                        List.of(
                                new EdgeToNodeCondition(null, null, null),
                                new EdgeToNodeCondition(
    ...
    public void testFitsSimpleValidityCheck() throws AccessDeniedException {
        ...
        QualifiedName startNodeId = new org.openprovenance.prov.vanilla.QualifiedName(BLANK_URI,
                "StoredSampleCon_r1_Spec", "blank");
        ICondition<INode> isMainActivity = new HasAttrQualifiedNameValue(ATTR_PROV_TYPE.getUri(),
                CPM_URI + "mainActivity");
        ICondition<INode> hasMetaReference = new HasAttr(ATTR_REFERENCED_META_BUNDLE_ID.getUri());
        ICondition<INode> isBackwardConnector = new HasAttrQualifiedNameValue(ATTR_PROV_TYPE.getUri(),
